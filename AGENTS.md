...
- `src/main.rs` — everything else: `slint::include_modules!()`, converts `data` ↔ Slint models, all `on_request_*` callback wiring (incl. File > Open / Import (Merge) / Save / Save As, which load or merge a user-picked `data.json` via `data::load_result`/`data::merge_projects`, persist via `AppContext::save_data_or_status`, and let Save As switch `ConfigData.data_dir` to the picked file's folder). Callbacks capture one shared `Rc<AppContext>` (data + config `RefCell`s; the JSON paths are derived from `ConfigData`, so they can never drift from the configured data directory) instead of cloning the pairs individually; `ui` is only ever captured as a weak handle to avoid an `Rc` cycle. Saves go through `AppContext::save_data_or_status`/`save_config_or_status`.
