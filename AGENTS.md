...
- `src/main.rs` — everything else: `slint::include_modules!()`, converts `data` ↔ Slint models, all `on_request_*` callback wiring (incl. File > New / Open / Import (Merge) / Save / Save As: New clears the data after the user picks where to save it via an `rfd` dialog, Open / Import load or merge a user-picked `data.json` via `data::load_result`/`data::merge_projects`, all saves persist via `AppContext::save_data_or_status`, and Save As switches `ConfigData.data_dir` to the picked file's folder). Callbacks capture one shared `Rc<AppContext>` (data + config `RefCell`s + the current data-file path, which starts as the `data.json` under the configured directory and follows File > Open / Save As / "Save Location"); `ui` is only ever captured as a weak handle to avoid an `Rc` cycle. Saves go through `AppContext::save_data_or_status`/`save_config_or_status`.
...
- **Data & settings split**: roadmap data (`RoadmapData.projects`) lives in `data.json` under `~/Documents/RoadMaps` by default, or under the user-chosen `ConfigData.data_dir` ("Save Location" in the settings window); app settings (`ConfigData.theme` / `ConfigData.language` / `ConfigData.data_dir` / `ConfigData.current_file`) live in `~/.RoadMapGenerator/config.json`. `config_dir()` resolves the home dir from `%USERPROFILE%` (Windows) / `$HOME` (elsewhere), falling back to `current_dir()` when it is unknown — never `current_exe()`. Both files are auto-saved after every mutation and on exit; the directory is created lazily on first save (`data::save`/`data::save_config`). Changing the data directory reloads from the new location (no migration) and makes it the current data file; File > Open / Save As / New likewise switch the current data file (so File > Save and the exit save write to the file the user is actually working on, not the configured default). On exit the current data file is recorded in `ConfigData.current_file`, and the next launch opens it again via `initial_data_file` (falling back to the configured directory when it is missing or no longer parses). `export.rs` reads the language from the process-wide `i18n::current()`, not from the data.
