...
- `src/main.rs` — everything else: `slint::include_modules!()`, converts `data` ↔ Slint models, all `on_request_*` callback wiring. Callbacks capture one shared `Rc<AppContext>` (data + config `RefCell`s + their JSON paths) instead of cloning the pairs individually; `ui` is only ever captured as a weak handle to avoid an `Rc` cycle.
