...
- `src/main.rs` — everything else: `slint::include_modules!()`, converts `data` ↔ Slint models, all `on_request_*` callback wiring (incl. File > Open / Import (Merge) / Save / Save As, which load or merge a user-picked `data.json` via `data::load_result`/`data::merge_projects`, persist via `AppContext::save_data_or_status`, and let Save As switch `ConfigData.data_dir` to the picked file's folder). Callbacks capture one shared `Rc<AppContext>` (data + config `RefCell`s + the current data-file path, which starts as the `data.json` under the configured directory and follows File > Open / Save As / "Save Location"); `ui` is only ever captured as a weak handle to avoid an `Rc` cycle. Saves go through `AppContext::save_data_or_status`/`save_config_or_status`.
